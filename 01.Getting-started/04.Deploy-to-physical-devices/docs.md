...
```bash
echo "$IP_OF_MENDER_SERVER_FROM_DEVICE docker.mender.io s3.docker.mender.io" | mender-artifact append <imgname>.sdimg:/etc/hosts -
```
...
Then you can check the file's contents by

```bash
mender-artifact cat <imgname>.sdimg:/etc/systemd/network/eth/network
```
...
```bash
echo "$IP_OF_MENDER_SERVER_FROM_DEVICE docker.mender.io s3.docker.mender.io" | mender-artifact append beaglebone_release_1_configured.mender:/etc/hosts
```
...
```bash
mender-artifact cat beaglebone_release_1_configured.mender:/etc/systemd/network/eth.network
```
